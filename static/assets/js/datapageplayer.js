...
//Get player data for selected player before letting the user pick from a button  
var player_route = d3.select("#selectedplayername").text().trim()
d3.json("/NBAData", function(nba_data){
    d3.json("/NCAAData", function(ncaa_data){
        console.log(player_route)
        var filteredNBA = nba_data.filter(playername => playername.Name == player_route)[0];
        var filteredNCAA = ncaa_data.filter(playername => playername.Name == player_route)[0];
